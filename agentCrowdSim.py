...
class Agent:
    ...
    def compute_path(self):
        ...
        self.path = world_collisions.compute_path((round(self.x_pos), round(self.y_pos)), (round(self.desired_location[0]), round(self.desired_location[1])))
    ...
    def shift_avoid_people(self):
        ...
        if n_people > 0:
            ...
            shift = [cos(avoidance_dir) * AGENT_SPACING_SHIFT * n_people, sin(avoidance_dir) * AGENT_SPACING_SHIFT * n_people]
            optimal_vector = [cos(self.desired_direction) * self.desired_step_size, sin(self.desired_direction) * self.desired_step_size]
            shifted_vector = [optimal_vector[0] + shift[0], optimal_vector[1] + shift[1]]
        ...
        for obstacle in agent_collisions.get_all_collision_considerations(self.collision_grid_pos):
            if self != obstacle:
                ...
                if self.hypothetical_distance_to(move, obstacle) < 2 * AGENT_RADIUS + AGENT_COMFORTABLE_SPACING:
                    people_factor -= 1/self.hypothetical_distance_to(move, obstacle)
                if self.distance_to(obstacle) < 2 * AGENT_RADIUS + AGENT_COMFORTABLE_SPACING:
...
class AgentCollisionManager:
    ...
    def __init__(self, x, y):
        self.squares = [[[] for _ in range(int(y/self.square_side))] for _ in range(int(x/self.square_side))]
...
class AstarNode:
    ...
    def __init__(self, pos, parent, goal):
        ...
        if self.parent == None:
            ...
            self.static_cost = self.parent.static_cost + sqrt((self.pos[0]-self.parent.pos[0])**2 + (self.pos[1]-self.parent.pos[1])**2)
